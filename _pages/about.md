...
I'm a PhD student at Huazhong University of Science and Technology, where I am a member of the <a href="https://hustvl.github.io/">HUST Vision Lab</a> under the supervision of <a href="https://xwcv.github.io/">Prof. Xinggang Wang</a>.
...
{% for n in news %}
  {% if forloop.index <= 5 %}
  <li><em>{{ n.date | replace: "-0", "." | replace: "-", "." }}</em>: &nbsp;{{ n.content | markdownify | remove: '<p>' | remove: '</p>' }}</li>
  {% else %}
  <li class='news-extra' style='display:none'><em>{{ n.date | replace: "-0", "." | replace: "-", "." }}</em>: &nbsp;{{ n.content | markdownify | remove: '<p>' | remove: '</p>' }}</li>
  {% endif %}
{% endfor %}
{% if news.size > 5 %}
